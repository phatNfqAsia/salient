...
<div class="container-wrap <?php echo ($fullscreen_header == true) ? 'fullscreen-blog-header': null; ?> <?php if($blog_type == 'std-blog-fullwidth' || $hide_sidebar == '1') echo 'no-sidebar'; ?>">
  <div class="post-featured-img">
    ...
    ?>
        <div class="container">
          <div class="col span_9 section-banner">
            <?php echo get_the_post_thumbnail($post->ID, 'wide', array('title' => '')); ?>
          </div>
        </div>
    <?php
        }
			...
			<?php if(have_posts()) : while(have_posts()) : the_post();
			    ...
			    if((empty($bg) && empty($bg_color)) && $fullscreen_header != true) { ?>
                    <?php
                        ...
                        $image = wp_get_attachment_image_src($post_thumbnail_id, 'wide');
                        $is_picture_fit = $image[3];
                    ?>
					<div class="row heading-title" data-header-style="<?php echo $blog_header_type; ?>">
                        <?php if ($is_picture_fit) { ?>
                            <div class="col span_9 section-banner"></div>
                        <?php } ?>
						<div class="col span_12 section-title blog-title">
                            ...
                            <div id="single-below-header">
								<span class="meta-author1 vcard author"><span class="fn"><?php echo __('By', NECTAR_THEME_NAME); ?> <?php the_author_posts_link(); ?></span></span><!--
								...
								<div id="single-meta" data-sharing="<?php echo ( !empty($options['blog_social']) && $options['blog_social'] == 1 ) ? '1' : '0'; ?>">
									<ul>
										...
										<li class="meta-comment-count">
											<a href="<?php comments_link(); ?>"><i class="icon-default-style steadysets-icon-chat"></i> <?php comments_number( __('', NECTAR_THEME_NAME), __('One Comment ', NECTAR_THEME_NAME), __('% Comments', NECTAR_THEME_NAME) ); ?></a>
										</li>
